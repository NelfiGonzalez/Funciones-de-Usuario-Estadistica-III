#Funcion de usuario regexpo.ErrorARMAv03() para el ajuste y pronostico de modelos de regresion exponenciales con error ARMA, hace un ajuste aproximado en el cual estima primero los parametros de las componentes estructurales
...
#residuos estructurales del modelo preliminar. El anterior procedimiento tambien es aplicado para la construccion de pronosticos puntuales.
#Intervalos de pronostico son construidos aproximando el error estandar del error de prediccion como la raiz cuadrada de la suma de la varianza estimada del error de prediccion en el error estructural
#y la varianza estimada (por metodo delta) de la estructura exponencial evaluada en los tiempos de prediccion
#Los argumentos son:
...
# level:         Nivel de confianza para intervalos de prediccion dado por un valor en (0,1), por defecto es 0.95.
...
#                con la funcion coef() aplicada al objeto R donde se guarde el resultado de la funcion regexpo.ErrorARMAv03().
...
#                sobre el objeto R donde se guarde el resultado de la funcion regexpo.ErrorARMAv03().
...
#                nombre_objeto$sigma2, donde 'nombre_objeto' es el nombre del objeto R donde se guarda el resultado de la funcion regexpo.ErrorARMAv03().
# forecast:      Objeto tipo  "mts", "ts", "matrix", "array" con los pronosticos puntuales y por intervalos de la respuesta para h=nrow(newdata) periodos despues del ajuste.


regexpo.ErrorARMAv03=function(respuesta,data,newdata,level=0.95,control=nls.control(),order= c(0L, 0L, 0L),seasonal=list(order = c(0L, 0L, 0L),period=NA),fixed=NULL,method=c("CSS-ML", "ML", "CSS"),optim.method="BFGS",optim.control = list()){
yt=respuesta
...
ytpron=ts(predict(modelo,newdata=newdata),freq=frequency(yt),start=start.pron)+ts(forecast(ciclos,h=h)$mean,freq=frequency(yt),start=start.pron)

#el vector de predictores en i esima prediccion, incluye al 1 como primera entrada
x0=cbind(1,as.matrix(newdata)) #matriz con filas siendo los puntos de prediccion o valores de variables explicatorias en la prediccion
mse=summary(modelo)$sigma^2 #estimacion varianza del error del modelo de regresion exponencial
hatexp=predict(modelo,newdata=newdata)
varbetas=as.matrix(vcov(modelo)) #matriz de varianzas covarianzas del vector de parametros estimados modelo exponencial
Sigmapred=(hatexp^2)*diag(x0%*%varbetas%*%t(x0)) #aproximacion varianza estructura estimada
se.res=predict(ciclos,n.ahead=h)$se
Sigmatot=Sigmapred+(se.res)^2
LI=ytpron-qt((1-level)/2,df=df,lower.tail=F)*sqrt(Sigmatot)
LS=ytpron+qt((1-level)/2,df=df,lower.tail=F)*sqrt(Sigmatot)
tablapron=ts(data.frame(forecast=ytpron,lower=LI,upper=LS),freq=frequency(yt),start=start.pron)
result=list(coefficients=tabla,fitted=ythat,residuals=resid,sigma2=sigma2,p=p2,forecast=tablapron)
result
}
