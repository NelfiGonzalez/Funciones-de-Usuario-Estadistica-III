#Función de usuario regexpo.ErrorARMA() para el ajuste y pronóstico de modelos de regresión expoenciales con error ARMA, hace un ajuste aproximado en el cual estima primero los parámetros de las componentes estructurales
...
regexpo.ErrorARMA=function(respuesta,names.param,data,newdata,control=nls.control(),order= c(0L, 0L, 0L),seasonal=list(order = c(0L, 0L, 0L),period=NA),method=c("CSS-ML", "ML", "CSS"),optim.method="BFGS",optim.control = list()){
yt=respuesta
...
ciclos=Arima(serie.et,order=order,seasonal=seasonal,include.mean=F,method=method,optim.method=optim.method,optim.control=optim.control)
